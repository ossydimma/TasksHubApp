...
export default function Page() {
  ...
  const successfulApiCall = () => {
    ...
    localStorage.removeItem("newtaskValues");
    setMessage("");
    router.push("/mytasks");
  };
  ...
  const validateForm = (): string | null => {
    ...
    if (
      taskValues.description.length > 200 &&
      taskValues.description.length < 3
    ) {
  ...
  const retrieveDraft = ()=> {
    ...
    if (draft) {
      const draftData = JSON.parse(draft) as TaskValuesType;
