...
export default function Page() {
  const { userInfo, isAuthenticated, setAccessToken, setUserInfo } = useAuth();
  ...
  useEffect(() => {
    ...
    const sendToken = async () => {
      try {
        const token = await SettingsServices.changeGoogleAccount(
          session.idToken
        );
  ...
  return (
    <div className="px-2 sm:px-10 pt-7 pb-20 md:pb-0 w-full h-auto md:h-full font-serif relative">
      {loading && (
        <div
      ...
      <div className={`${disablePage ? `hidden` : `block`}  pt-20`}>
        <ul className="flex flex-col gap-6 px-4 sm:px-2">
          {/* ------------------------USERNAME------------------ */}
          <li
      ...
      {disablePage && (
        <div className={` h-[75vh] mx-4 sm:mx-0`}>
          <div className={`text-lg font-bold ml-5 sm:-mt-4 hidden sm:block `}>
            <span
          ...
          <div className={`h-full flex justify-center  items-center`}>
            <div className=" pt-8 pb-12 px-10 md:px-6 xl:px-10 z-10 bg-white shadow-xl h-auto top-3 md:top-8  w-full md:w-[45%] rounded-2xl ">
      ...
      {showChangeContact && (
        <ChangeContact setShowChangeContact={setShowChangeContact} />
      )}
    </div>
  );
}
